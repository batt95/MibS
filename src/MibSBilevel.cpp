...
{
	bool warmStartLL(model_->MibSPar_->entry(MibSParams::warmStartLL));
	int maxThreadsLL(model_->MibSPar_->entry(MibSParams::maxThreadsLL));
	int whichCutsLL(model_->MibSPar_->entry(MibSParams::whichCutsLL));
	std::string feasCheckSolver(model_->MibSPar_->entry(MibSParams::feasCheckSolver));
	MibSBranchingStrategy branchPar = static_cast<MibSBranchingStrategy>(model_->MibSPar_->entry(MibSParams::branchStrategy));
	int computeBestUBWhenXVarsInt(model_->MibSPar_->entry(MibSParams::computeBestUBWhenXVarsInt));
	int computeBestUBWhenLVarsInt(model_->MibSPar_->entry(MibSParams::computeBestUBWhenLVarsInt));
	int computeBestUBWhenLVarsFixed(model_->MibSPar_->entry(MibSParams::computeBestUBWhenLVarsFixed));
	int useLinkingSolutionPool(model_->MibSPar_->entry(MibSParams::useLinkingSolutionPool));
	...
	int useImprovingDirectionPool(model_->MibSPar_->entry(MibSParams::useImprovingDirectionPool));

	bool  shouldSolveLower(true);
	...
	if (shouldSolveLower && !isContainedInLinkingPool_)
	{
		...
		{
			dynamic_cast<OsiCbcSolverInterface *>(lSolver)->getModelPtr()->messageHandler()->setLogLevel(0);
		}
		...
		if ((feasCheckSolver == "SYMPHONY") && (sym_is_time_limit_reached(dynamic_cast<OsiSymSolverInterface *>(lSolver)->getSymphonyEnvironment())))
		{
			shouldPrune_ = true;
			storeSol = MibSNoSol;
			goto TERM_CHECKBILEVELFEAS;
		...
		else if (!lSolver->isProvenOptimal())
		{
			...
			if (useLinkingSolutionPool && isLinkVarsIntegral_)
			{
				// step 10
				// Adding x_L to set E
				addSolutionToSeenLinkingSolutionPool(MibSLinkingPoolTagLowerIsInfeasible, shouldStoreValuesLowerSol, 0.0);
			}
			if (isLinkVarsFixed_)
			{
				useBilevelBranching_ = false;
		 ...
		 (tagInSeenLinkingPool_ == MibSLinkingPoolTagUBIsSolved)))
	{
		...
		if (!shouldPrune_)
		{
				 ...
				 ((computeBestUBWhenLVarsFixed == PARAM_ON) && (isLinkVarsFixed_))))
			{
				...
				if ((feasCheckSolver == "SYMPHONY") && (sym_is_time_limit_reached(dynamic_cast<OsiSymSolverInterface *>(UBSolver)->getSymphonyEnvironment())))
				{
					shouldPrune_ = true;
					goto TERM_CHECKBILEVELFEAS;
				}
